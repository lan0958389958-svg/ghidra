...
# Ghidra 軟體逆向工程框架
Ghidra 是由 [`美國國家安全局`][nsa] 研究部門開發和維護的逆向工程軟體 (SRE) 框架。    
此框架包括反彙編、彙編、反編譯、圖形化和腳本編寫，以及數百種其他功能的高階軟體分析工具，可分析編譯後的程式碼。   
且支援 Windows、macOS 和 Linux 等多種平台，Ghidra 支援多種處理器指令集和可執行檔格式，並可在使用者互動和自動化模式下執行。  
使用者還可以利用 Java 或 Python 開發自己的 Ghidra 擴充元件和/或腳本。  
為支援`美國國家安全局`的網路安全任務，Ghidra 旨在解決複雜 SRE 工作中的規模和團隊協作問題，並提供可客製化和可擴展的 SRE 研究平台。  
`美國國家安全局`已將 Ghidra SRE 功能應用於各種問題，包括分析惡意程式碼，為尋求更深入了解網路和系統潛在漏洞的 SRE 分析師提供深入見解。  
如果您是美國公民，對此專案感興趣，希望為`美國國家安全局`開發 Ghidra 和其他網路安全工具，以幫助保護我們的國家及其盟友，請考慮申請我們的 [職位][career]。  

## 安全警告
**警告：** 某些版本的 Ghidra 存在已知的安全漏洞，使用前請閱讀 Ghidra 的 [安全公告][security]，以便充份了解您可能受到的影響。

## 安裝
要安裝官方預建的多平台 Ghidra 版本：
* 安裝 [JDK 21 64 位元][jdk]
* 下載 Ghidra [發行檔案][releases]
  - **注意：** 官方多平台發行檔案的名稱為 `ghidra_<version>_<release>_<date>.zip`，可在「Assets」下拉選單中找到，下載前請確認名稱為「Source Code」，其它的任何檔名都是不正確的。
* 解壓縮 Ghidra 發行檔案
  - **注意：** 請勿在現有安裝之上解壓縮
* 啟動 Ghidra：`./ghidraRun` (Windows 為 `ghidraRun.bat`)
  - 或啟動 [PyGhidra][pyghidra]：`./support/pyghidraRun` (Windows 為 `support\pyghidraRun.bat`)

相關安裝和執行，請參閱 Ghidra 安裝目錄根目錄下的 [入門文件][gettingstarted]，了解更多的資訊和故障排除提示。
...
cd ghidra-master
```
**注意：** 如果希望克隆 GitHub 儲存庫，而不是下載壓縮的原始碼，請執行：`git clone https://github.com/NationalSecurityAgency/ghidra.git`
...
Ghidra 相關佈署，請閱讀 [開發人員指南][devguide]，瞭解更多詳細資訊。  
如杲佈署困難，請閱讀 [已知問題][known-issues] 以獲取可能的解決方案。

## 開發

### 使用者腳本和擴充功能
Ghidra 安裝支援使用者透過 Eclipse 的 *GhidraDev* 外掛程式編寫自訂腳本和擴充功能。  
該外掛程式及其相應說明可在 Ghidra 版本中的 `Extensions/Eclipse/GhidraDev/` 或 [此連結][ghidradev] 找到。
或者，可以透過點擊腳本管理器中的 Visual Studio Code 圖示來使用 Visual Studio Code 編輯腳本。  
功能齊全的 Visual Studio Code 專案可以從 Ghidra CodeBrowser 視窗中的 _工具 -> 建立 VSCode 模組專案_ 建立。
